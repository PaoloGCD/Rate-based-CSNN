...
# IMPORT PARAMETERS
parameters = "../../params/CSNN_400-direct.xml"
result_path = "../../results/CRBA-400-0/"
test_number = 50000
if len(sys.argv) == 4:
...
print 'EXPERIMENT', result_path, test_number
...
# LOAD DATA
load_threshold = (np.load(result_path + "partial_threshold_%06d.npy" % test_number) - 120) * 15 / 160 + 25
load_weights = np.load(result_path + "partial_weight_%06d.npy" % test_number)*78.

print load_threshold.min(), load_threshold.max()
print load_weights.min(), load_weights.max()
...
neuron_eqs_e = '''
        dv/dt = ((v_rest_e-v) + ge*(65.*mV-v) + gi*(-35.*mV-v)) / (tau_v)   : volt
        dge/dt = -ge/tau_ge                                                 : 1
        dgi/dt = -gi/tau_gi                                                 : 1
...
neuron_groups['spiking'].threshold = load_threshold * b.mV
...
weight_matrix_input = load_weights.transpose()
...
while i < number_samples:
    ...
    if number_spikes < 5:
        ...
        if i % 10 == 0 and i > 0:
            ...
            print 'runs done:', i, 'of', int(number_samples), ', required time:', end - start, number_spikes
            start = time.time()
...
np.save(result_path + "spiking_d_testing_rates_%06d" % test_number, result_spike_activity)
np.save(result_path + "spiking_d_testing_time_%06d" % test_number, total_time)
...
load_labels = np.load(result_path + "partial_label_%06d.npy" % test_number)
labels = load_labels
...
np.save(result_path + "spiking_d_test_accuracy_%06d" % test_number, accuracy)
