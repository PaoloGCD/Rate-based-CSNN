...
labeling_print_step = int(params['params']['labeling']['print_step'])
labeling_number_samples = int(params['params']['labeling']['number_samples'])
...
training_x = training_set_x[:labeling_number_samples, :]
training_y = training_set_y[:labeling_number_samples]
...
result_spike_activity = np.zeros((labeling_number_samples, spiking_neurons))

total_start = time.time()
start = time.time()

i = 0
while i < labeling_number_samples:
    ...
    if number_spikes < 5:
        ...
        if (i+1) % labeling_print_step == 0:
            end = time.time()
            print 'runs done:', i+1, 'of', int(labeling_number_samples), ', required time:', end - start
            start = time.time()
...
np.save(result_path + '/labeling_time', total_time)
...
for i in xrange(number_classes):
    class_spike_activity = result_spike_activity[training_y[:labeling_number_samples].reshape(-1) == i, :]
