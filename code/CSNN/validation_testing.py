...
while i < number_samples:
    ...
    if number_spikes < 5:
        ...
        if (i+1) % validation_print_step == 0:
            ...
            print 'runs done:', i + 1, 'of', int(number_samples), ', required time:', end - start
            start = time.time()
...
np.save(result_path + '/validation_testing_time_%06d' % partial_number, total_time)
...
load_labels = np.load(result_path + '/validation_labeling_labels_%06d.npy' % partial_number)
labels = load_labels
...
# evaluate accuracy
difference = training_y.reshape(-1) - predicted_labels
correct = len(np.where(difference == 0)[0])
accuracy = correct/float(number_samples)

np.save(result_path + '/validation_testing_accuracy_%06d' % partial_number, accuracy)
